...
model.Max_Shifting_Capability = pyo.Param(model.T, within=pyo.NonNegativeReals, initialize=0.40)   # Max percentage of flexible load that can be shifted (Percentage)
...
def shifted_load_limit_rule(model, t, delta):
    if 0 <= t + delta < len(model.T):
        return model.P_flexible[t + delta] + model.P_shift[t] * model.y_shift[t, delta] <= 1.4 * model.P_flexible[t + delta]
    else:
        return pyo.Constraint.Skip
...
if results.solver.termination_condition == 'optimal':
    ...
    folder_name = current_time.strftime("OptimizationResults_WINTER_40PShift_6HLimit_%Y-%m-%d_%H-%M-%S")
    ...
    file_name = current_time.strftime("OptimizationResults_WINTER_40PShift_6HLimit_%Y-%m-%d_%H-%M-%S.xlsx")
    ...
    scenario_label = "40% Shift, 6H Limit"  # Update this based on the scenario
