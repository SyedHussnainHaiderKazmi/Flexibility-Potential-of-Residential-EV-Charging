...
model.Delta_P_shift = pyo.Var(model.T, domain=pyo.Integers, bounds=(-48, 48))     # Integer shifts within ±6 hours

# Binary variable y_shift[t, delta] that determines whether P_shift[t] is moved to interval t+delta
model.y_shift = pyo.Var(model.T, range(-48, 49), domain=pyo.Binary)
    ...
    return sum(
        model.C_t[t] * (model.P_fixed[t] + model.P_flexible[t] - model.P_shift[t] 
            + sum(model.P_shift[t - delta] * model.y_shift[t - delta, delta]  
                  for delta in range(-48, 49) if 0 <= t - delta < len(model.T))
...
def upper_power_limit_rule(model, t):
    return (model.P_fixed[t] + model.P_flexible[t] - model.P_shift[t]  
        + sum(model.P_shift[t - delta] * model.y_shift[t - delta, delta] for delta in range(-48, 49) if 0 <= t - delta < len(model.T))  # Add received flexible load
...
def shift_assignment_rule(model, t):
    return sum(model.y_shift[t, delta] for delta in range(-48, 49) if 0 <= t + delta < len(model.T)) == 1
...
model.shifted_load_limit = pyo.Constraint(model.T, range(-48, 49), rule=shifted_load_limit_rule)
...
def enforce_shift_rule(model, t):
    return model.P_shift[t] == sum(model.P_shift[t] * model.y_shift[t, delta] for delta in range(-48, 49) if 0 <= t + delta < len(model.T))
...
def delta_p_shift_definition_rule(model, t):
    return model.Delta_P_shift[t] == sum(delta * model.y_shift[t, delta] for delta in range(-48, 49)) * model.z_shift[t]
...
if results.solver.termination_condition == 'optimal':
    ...
    folder_name = current_time.strftime("OptimizationResults_WINTER_50PShift_12HLimit_%Y-%m-%d_%H-%M-%S")
    ...
    results_dict = {
        ...
        'P_flexible_post_shift': [P_flexible[t] - model.P_shift[t]() +  
            sum(model.P_shift[t - delta]() * model.y_shift[t - delta, delta]() for delta in range(-48, 49) if 0 <= t - delta < len(model.T))
            for t in model.T],
    ...
    file_name = current_time.strftime("OptimizationResults_WINTER_50PShift_12HLimit_%Y-%m-%d_%H-%M-%S.xlsx")
    ...
    scenario_label = "50% Shift, 12H Limit"  # Update this based on the scenario
